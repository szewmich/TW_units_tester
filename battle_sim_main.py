...
def run_sim(ID_A, ID_B):
    ...
    pursuit_damage_factor = 0.2         # factor of damage inflicted on fleeing unit after it is routed (0.2 = 20% of total hp left)
    ...
    def get_rout_dmg(morale, hp):
        """Calculate the damage needed to rout a unit based on its morale."""
        if morale < 15:
            rout_dmg = hp * 0.20
        if morale >= 15 and morale < 30:
            rout_dmg = hp * (0.20 + (morale - 15) / (30 - 15) * 0.20)  # Linear interpolation between 20% and 40%
        if morale >= 30 and morale < 55:
            rout_dmg = hp * (0.40 + (morale - 30) / (55 - 30) * 0.20)  # Linear interpolation between 40% and 60%
        if morale >= 55 and morale < 80:
            rout_dmg = hp * (0.60 + (morale - 55) / (80 - 55) * 0.20)  # Linear interpolation between 60% and 80%
        if morale >= 80:
            rout_dmg = hp * 1.0
        
        # Ensure rout damage is an integer
        return int(rout_dmg)
    
    A_rout_dmg = get_rout_dmg(A.morale, A.hp)  # Calculate damage needed to rout unit A
    B_rout_dmg = get_rout_dmg(B.morale, B.hp)  # Calculate damage needed to rout unit B
    ...
    while games_played < n_games:
        ...
        while ongoing:
            ...
            # The assumption is that the unit gets routed if its accumulated damage (CD) reaches the rout damage threshold
            # 
            # ROUTING at HP threshold
            if B.CD >= B_rout_dmg :                                                 # If B receives enough damage to get routed
                if (A_rout_dmg - A.CD) > (B_rout_dmg - B.CD):                       # If A remaining hp is greater than B remaining hp (in case they are both negative)
                    A_wins = A_wins + 1
                    ongoing = False                                                 # No more attack exchange rounds will occur in current game
                    A_perc_hp_left = (A_rout_dmg - A.CD) / A_rout_dmg               # Ratio of unit's remaining useful hp vs unit hp damage to rout
                    A_value = B.cost                                                # Value generated for A = cost of unit B (which got routed = killed completely)
                    B_value = (1 - A_perc_hp_left) * A.cost                         # Value generated for B - proportional to A cost and percentage of useful hp damage dealt
                    B_pursuit_casaulties = pursuit_damage_factor * (B.hp - B.CD)
            if A.CD >= A_rout_dmg:                                                  # Same as above but for A switched with B
                if (B_rout_dmg - B.CD) > (A_rout_dmg - A.CD):
                    B_wins = B_wins + 1
                    ongoing = False
                    B_perc_hp_left = (B_rout_dmg - B.CD) / B_rout_dmg
                    B_value = A.cost
                    A_value = (1 - B_perc_hp_left) * B.cost
                    A_pursuit_casaulties = pursuit_damage_factor * (A.hp - A.CD)
    ...
    A_remaining_total_hp_perc = round((A.hp - A.CD - A_pursuit_casaulties) / A.hp * 100, 0)
    B_remaining_total_hp_perc = round((B.hp - B.CD - B_pursuit_casaulties) / B.hp * 100, 0)
    ...
    if melee_switch == 1:
        ...
        print (f'{A.name} win count = {A_wins} with average {A_remaining_total_hp_perc} % hp left for unit, killing {A_value_avg} gold worth of value ({A_perc_hp_left_avg} % remaining hp before rout)')
        print (f'{B.name} win count = {B_wins} with average {B_remaining_total_hp_perc} % hp left for unit, killing {B_value_avg} gold worth of value ({B_perc_hp_left_avg} % remaining hp before rout)')
